...
<html><head>
    ...
    <title>Heatbud | Order Payment</title>
...
<body style="position: relative">
	...
	<div class="profileLabel" style="width:87%; margin-left:4%; margin-top:30px">
		<span style="margin-left:36px; border-bottom:5px solid rgb(139, 197, 62); padding-bottom:4px">
			<span>Pay </span>
			<span style="font-weight:bold">${po.bloggerId}</span>
			<span> an amount of </span>
			<span style="font-weight:bold">$${po.price}</span>
			<span> toward </span>
			<span style="font-weight:bold">${po.postType}</span>
		</span>
	</div>
	<div class="profileDiv" style="width:87%; margin-left:4%; white-space:normal; padding-top:5%; padding-bottom:5%; text-align:center">
		<c:if test="${not empty ERROR}">
			<div style="margin-left:208px; margin-top:10px; color:red">${ERROR}</div>
		</c:if>
		<c:if test="${empty ERROR}">
			<div style="color:#909090; margin-bottom:10px">We use Stripe for secure payments. Visit https://stripe.com to learn more.</div>
			<form action="/user/order-process/${po.orderHandler}/${po.price}" method="POST">
			  <script
			    src="https://checkout.stripe.com/checkout.js" class="stripe-button"
			    data-key="pk_live_CIKOeylggc4i1ovHul34q09Z"
			    data-amount="${po.price*100}"
			    data-name="Heatbud LLC"
			    data-description="https://www.heatbud.com/user/order/${po.orderHandler}"
			    data-image="https://stripe.com/img/documentation/checkout/marketplace.png"
			    data-zip-code="true"
			    data-locale="auto">
			  </script>
			</form>
			<div style="color:#909090; margin-top:10px">You do not need to have a Stripe account for this transaction.</div>
		</c:if>
	</div>
	<%-- End page content --%>
