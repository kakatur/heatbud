...
<html><head>
    ...
    <title>Heatbud | Page Keywords</title>
	...
	<script src="/resources/js/heatbud-page-keywords-min.js?20180530"></script>
...
<body style="position:relative">
	...
	<div class="profileLabel h1" style="width:87%; margin-top:10px; margin-left:4%">
		<span style="margin-left:26px; border-bottom:5px solid rgb(139, 197, 62); padding-bottom:4px">KEYWORDS FOR ${entity.entityName}</span>
	</div>
	<div class="profileDiv" style="white-space:normal; width:87%; margin-left:4%">
		<div style="margin-left:26px">
			<div style="margin-top:20px; margin-bottom:30px">Bloggers will use these keywords in their blog posts.</div>
			<script>
				var TL = new Array();
			</script>
			<c:if test="${empty entityTagsList}">
				<div id="page-tags" style="color:#61cf81; margin-left:26px; margin-bottom:30px">No Keywords.</div>
			</c:if>
			<c:if test="${not empty entityTagsList}">
				<div id="page-tags" style="width:60%; margin-bottom:30px">
					<c:forEach var="et" items="${entityTagsList}" varStatus="counter">
						<script>
							TL.push("${et.id}");
						</script>
						<div id="${et.id}" style="display:inline-block; border-radius:14px; border:1px solid #61cf81; padding:4px 10px 4px 10px; margin-top:6px; margin-right:6px; font-family:Arial; font-size:16px; color:#365899">
							<span id="${et.id}-value">${et.tag}</span>&nbsp;&nbsp;&nbsp;
							<c:if test="${canEdit == 'Y'}">
								<a href="javascript:" style="font-size:1em; color:#8f8f82" onclick="deletePageTag('${et.id}')" title="Delete this keyword">&nbsp;x&nbsp;</a>
							</c:if>
						</div>
					</c:forEach>
				</div>
			...
			<c:if test="${canEdit == 'Y'}">
				...
				<div class="textDiv">
					<input id="tag-input" style="border-radius:14px; border:1px solid #61cf81" placeholder="example: handicrafts" class="inputText"/>&nbsp;
					<button id="tag-button" onclick="addPageTag()" class="activeButton">add</button>
				</div>
				<div id="tag-error" style="margin-top:6px; color:red; margin-bottom:40px">&nbsp;</div>
