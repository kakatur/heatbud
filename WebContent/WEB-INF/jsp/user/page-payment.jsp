...
<html><head>
    ...
    <title>Heatbud | Page Payment</title>
	...
	<script src="/resources/js/heatbud-page-payment-min.js?20180530"></script>
...
<body style="position: relative">
	...
	<input id=paymentHandlerHidden type=hidden value="${paymentHandler}"/>

	<div class="profileLabel" style="width:87%; margin-top:10px; margin-left:4%">
		<span style="font-weight:bold; margin-left:36px; border-bottom:5px solid rgb(139, 197, 62); padding-bottom:4px">
			<span>PAY $${ppp.amount} toward "${ppp.productType}" package</span>
			<c:if test="${not empty ppp.coupon}">
				<span> using coupon code ${ppp.coupon}</span>
			</c:if>
			<c:if test="${ppp.endDate == 1}">
				<span> (automatic payments)</span>
			</c:if>
			<c:if test="${ppp.endDate == 0}">
				<span> (one-time payment)</span>
			</c:if>
		</span>
	</div>
	<div class="profileDiv" style="white-space:normal; width:87%; margin-left:4%; padding-top:10%; padding-bottom:10%; text-align:center">
		<c:if test="${empty ERROR}">
			<c:if test="${ppp.endDate == 1}">
               	<form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top">
                	<input type="hidden" name="cmd" value="_xclick-subscriptions">
               		<input type="hidden" name="business" value="paypal@heatbud.com">
                	<input type="hidden" name="landing_page" value="billing">
               		<input type="hidden" name="item_name" value="Heatbud Recurring Payments for ${ppp.productType}">
	                <input type="hidden" name="a3" value="${ppp.amount}"/>
					<input type="hidden" name="p3" value="1"/>
					<c:if test = "${fn:contains(ppp.productType, 'monthly')}">
						<input type="hidden" name="t3" value="M"/>
					</c:if>
					<c:if test = "${fn:contains(ppp.productType, 'yearly')}">
						<input type="hidden" name="t3" value="Y"/>
					</c:if>
					<input type="hidden" name="src" value="1"/>
					<input type="hidden" name="no_note" value="1"/>
	                <input type="hidden" name="return" value="https://www.heatbud.com/user/page-payment-success/${paymentHandler}"/>
	                <input type="hidden" name="cancel_return" value="https://www.heatbud.com/user/page-payment-error/${paymentHandler}"/>
					<input style="cursor:pointer" type="image" src="https://www.paypalobjects.com/en_US/i/btn/btn_paynowCC_LG.gif" name="submit" alt="PayPal - The safer, easier way to pay online!">
					<img alt="Pay using a credit card" border="0" src="https://www.paypalobjects.com/en_US/i/scr/pixel.gif" width="1" height="1">
				</form>
				<div style="color:#909090; margin-top:20px">You will be prompted to create a Paypal account if you don't already have one.</div>
			</c:if>
			<c:if test="${ppp.endDate == 0}">
               	<form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top">
	                ...
	                <input type="hidden" name="business" value="paypal@heatbud.com">
	                <input type="hidden" name="landing_page" value="billing">
	                <input type="hidden" name="item_name" value="Heatbud One-time Payment for ${ppp.productType}">
	                <input type="hidden" name="amount" value="${ppp.amount}"/>
	                <input type="hidden" name="no_note" value="1"/>
	                <input type="hidden" name="return" value="https://www.heatbud.com/user/page-payment-success/${paymentHandler}"/>
	                <input type="hidden" name="cancel_return" value="https://www.heatbud.com/user/page-payment-error/${paymentHandler}"/>
	                <input style="cursor:pointer" type="image" src="https://www.paypalobjects.com/en_US/i/btn/btn_paynowCC_LG.gif" name="submit" alt="PayPal - The safer, easier way to pay online!">
                ...
                <div style="color:#909090; margin-top:20px">We use Paypal for secure payments, but you don't need to have a Paypal account for this transaction.</div>
			</c:if>
			<div style="margin-top:20px">
				<span>Want to use or change coupon code? Change payment type? Change of mind?</span>
				<span> <a href="javascript:" onclick="cancelPayment()">Cancel this transaction</a> and go to the page payments page.</span>
			</div>
		</c:if>
		<div id="page-payment-error" style="margin-left:208px; margin-top:10px; color:red">${ERROR}</div>
	</div>
	<%-- End page content --%>
